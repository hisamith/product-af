...
function _replace_paas_as(){
    ...
    do
        ...
        sudo mkdir -p ${TMP_DIR} && sudo chmod -R 777 ${TMP_DIR}"

        echo -e "Copying from ${BACKUP_DIR}/${folder} to ubutnu@${PASS_HOST_IP}:${TMP_DIR}"
        scp -i ${PEM_PATH} -r ${BACKUP_DIR}/${folder}/* ${FULL_HOST}:${TMP_DIR}/

        # Moving data to correct directory and and remove temp directory
        echo -e "Moving from ${TMP_DIR} to ${SOURCE}/${folder}/"
        ssh -i ${PEM_PATH} -l ubuntu ${PASS_HOST_IP} "sudo mv ${TMP_DIR}/* ${SOURCE}/${folder}/ && \
        sudo rm -rf ${TMP_DIR}"
    done
    cd ${UPGRADE_HOME}
}